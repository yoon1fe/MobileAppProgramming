            ...
            if(v.getId() == R.id.confirm)       //확인 눌렀을때
            {
                ...
                for(int j = 0; j<time; j++)
                {
                    if(schedule[chooseDay][j] == 1 && flag == 0)
                    {
                        ...
                        text_time = start + ":00시 ~ ";
                        flag = 1;
                        helper.insertTimeTable345(database, userID, chooseDay, j);
                    }
...
//        }
        ...
        for(int i = 0; i<day; i++)
        {
            for(int j = 0; j<time; j++)
            {
                sql = "SELECT id FROM "+ helper.tableName345 + " WHERE day = " + i +  " and time = " + j ;
                cursor = database.rawQuery(sql, null);
        ...
        for(int i=0;i<3;i++){
            ...
            btn[0][i].setText("자료구조\n김용태");
            schedule[0][i] = 2;
        }
        for(int i=1;i<4;i++){
            btn[1][i].setBackgroundColor(getResources().getColor(R.color.colorPrimaryDark));
            btn[1][i].setText("자료구조특론\n정기숙");
            schedule[1][i] = 2;
        }

        System.out.println("In fragment 345");
        for(int i = 0; i<day; i++)
        {
            for(int j = 0; j<time; j++)
            {
