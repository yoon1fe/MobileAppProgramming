 ...
 */
public class Schedule_342Fragment extends Fragment {
            ...
            if(v.getId() == R.id.confirm)       //확인 눌렀을때
            {
                ...
                for(int j = 0; j<time; j++)
                {
                    if(schedule[chooseDay][j] == 1 && flag == 0)
                    {
                        ...
                        text_time = start + ":00시 ~ ";
                        flag = 1;
                        helper.insertTimeTable342(database, chooseDay, j);
                    }
                    else if(schedule[chooseDay][j] == 1 && flag == 1)
                    {
                        ...
                        if(j == 5)
                        {
                            ...
                            helper.insertRegistration(database, userID, 342, chooseDay, start, end);
                            flag = 0;
                        }
                    }
                    ...
                    for(int j = 0; j<time; j++)
                    {
                        if(v.getId() == btn[i][j].getId())
                        {
                            ...
                            if(schedule[i][j] == 1){
                                ...
                                //수업이 있음
                                Toast toast = Toast.makeText(context, "수업이 있습니다.", Toast.LENGTH_SHORT);
                                toast.show();
                            }
                            else{
                                chooseDay = i;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        for(int i = 0; i<day; i++)
        {
            for(int j = 0; j<time; j++)
            {
                sql = "SELECT * FROM "+ helper.tableName342 + " WHERE day = " + i +  " and time = " + j ;
                cursor = database.rawQuery(sql, null);
        ...
        //수업 시간 표시, schedule에 2로 표시
        for(int i=0;i<3;i++){
            btn[0][i].setBackgroundColor(getResources().getColor(R.color.colorPrimaryDark));
            btn[0][i].setText("이산수학\n최현석");
            schedule[0][i] = 2;
        }

        System.out.println("In fragment 342");
        for(int i = 0; i<day; i++)
        {
            for(int j = 0; j<time; j++)
            {
